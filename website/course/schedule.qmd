...
if(sum(links) > 0) {
	# tmp <- paste0(' <a href="', schedule[links,]$Link, '">', fontawesome::fa('readme'), '</a>')
	# tmp <- gsub('\\n', '', tmp)
...
if(sum(slides) > 0) {
	slide_links <- paste0(
		...
		fontawesome::fa('safari', fill = 'black'), '</a>',
		'&nbsp;&nbsp;',
		'<a href="', slides_uri, schedule[slides,]$Slides, '.pdf" target="_new">', 
		fontawesome::fa('file-pdf', fill = 'black'), '</a>'
	)
	slide_links <- gsub('\\n', '', slide_links)
...
if(sum(video) > 0) {
	video_links <- paste0(
		...
		fontawesome::fa('youtube', fill = 'black'), '</a>'
	)
	video_links <- gsub('\\n', '', video_links)
